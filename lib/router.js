...
PostsListController = FastRender.RouteController.extend({
  ...
  waitOn: function () {
    ...
    this._terms = {
      ...
      locLatMin: Session.get("locLatMin"),
      locLatMax: Session.get("locLatMax"),
      locLongMin: Session.get("locLongMin"),
      locLongMax: Session.get("locLongMax"),
      selectedMkr: Session.get("selectedMkr")
    };
...
Router.map(function() {
  ...
  this.route('post_submit', {
    path: '/submit/:catId?',
    data: function() {
      if(typeof this.params.catId === "undefined")
        cat = false;
      else
        cat = Categories.findOne({'slug': this.params.catId})._id;
      return {catId: cat};
    }
  });
